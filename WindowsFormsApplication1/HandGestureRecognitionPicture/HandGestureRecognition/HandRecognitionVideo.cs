...
namespace HandGestureRecognition
{
    ...
    public class HandRecognitionVideo
    {
        ...
        public HandRecognitionVideo(string video_path)
        {
            try
            {
                grabber = new Emgu.CV.Capture(video_path);
                grabber.QueryFrame();
        ...
        {
            while (( currentFrame = grabber.QueryFrame() ) != null) 
            {
        ...
        static private void ExtractContourAndHull(Image<Gray, byte> skin)
        {
        ...
        static private void DrawAndComputeFingersNum()
        {
            int fingerNum = 0;
        ...
        static private void write_score()
        {
            int min_tip_x;
            int min_tip_y;
            int min_hole_x;
            ...
            using (XmlWriter writer = XmlWriter.Create("points.xml"))
            {
                ...
                foreach (fingers finger in frames)
                {
                    ...
                    if (finger.fingertips.Count() != 0)
                    {
                        min_tip_x = finger.fingertips.Min(c => c.X);
                        min_tip_y = finger.fingertips.Min(c => c.Y);
                    }
                    else
                    {
                        min_tip_x = 0;
                    ...
                    if (finger.fingerholes.Count() != 0)
                    {
                        min_hole_x = finger.fingerholes.Min(c => c.X);
                        min_hole_y = finger.fingerholes.Min(c => c.Y);
                    }
                    else
                    {
                        min_hole_x = 0;
